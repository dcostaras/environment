...
** TODO add :lexical t globally to all emacs-lisp src blocks
...
#+name: org-api
#+begin_src emacs-lisp
(defun my--unfold-heading (heading)
...
**** Babel
:PROPERTIES:
:header-args+: :noweb-ref org-babel
:END:
...
**** Export
:PROPERTIES:
:header-args+: :noweb-ref org-export
:END:
...
**** Tangling
:PROPERTIES:
:header-args+: :noweb-ref org-tangle
:END:

***** Macros
:PROPERTIES:
:header-args+: :noweb-ref none
:END:
...
#+begin_src emacs-lisp
(with-current-buffer "rc.org"
  (message "%s" (current-buffer))
  ;; (remove-hook 'after-save-hook #'my--tangle-on-save nil 'buffer-local)
  ;; (add-hook 'after-save-hook #'my--tangle-on-save nil 'buffer-local)
  org-ctrl-c-ctrl-c-final-hook
  (remove-hook! 'org-ctrl-c-ctrl-c-final-hook :local #'my--tangle-on-save)
  (add-hook! 'org-ctrl-c-ctrl-c-final-hook :local #'my--tangle-on-save)

  (remove-hook! 'org-ctrl-c-ctrl-c-hook :local #'my--tangle-on-save)
  (add-hook! 'org-ctrl-c-ctrl-c-hook :local #'my--tangle-on-save)
  )
***** Environment

#+name: environment-file-name
#+begin_src emacs-lisp :results pp :noweb-ref none
(buffer-file-name)
#+end_src

#+begin_src emacs-lisp :noweb yes
(defvar my--environment-file-name <<environment-file-name()>>)
#+end_src
...
(defun >tangle-environment ()
  (interactive)
  (save-excursion
    (with-current-buffer (find-file my--environment-file-name)
      (save-buffer)
      (let* ((org-babel-exp-code-template "#+begin_src %lang :shebang %shebang :tangle %tangle%switches%flags\n%body\n#+end_src")
             (org-babel-default-header-args (cons '(:shebang . "") org-babel-default-header-args))
             (temp-org-file-name (org-org-export-to-org)))
        (org-babel-tangle-file temp-org-file-name)
        ;; (delete-file temp-org-file-name)
        ))))
#+end_src

#+begin_src emacs-lisp :eval no :noweb-ref none
(add-hook! 'after-save-hook :local #'my--tangle-environment nil 'buffer-local)
#+end_src

****** TODO make tangling the env async

**** org-tempo
#+include: ./org-tempo-templates.org
...
#+begin_src emacs-lisp :tangle ~/.config/emacs/.local/cache/templates
...
#+begin_src emacs-lisp :var bindings=lispy-bindings :colnames yes
(eval
 (append
...
#+begin_src emacs-lisp :noweb yes :tangle ~/.doom.d/packages.el
;; -*- no-byte-compile: t; -*-
;;; $DOOMDIR/packages.el
...
*** Vanilla
:PROPERTIES:
:header-args:emacs-lisp: :tangle ~/.config/rigpa-emacs/init.el
:END:
...
#+name: set-env
#+begin_src emacs-lisp :var env-block='()

...
(use-package symex
  ...
  (defun symex-clojure-comment-remaining-reader-macro ()
    "Comment out remaining symexes at this level using clojure(script)'s comment reader macro."
    (interactive)
    (let ((count (symex--remaining-length)))
      (my-symex-comment-reader-macro count)))

  (defun symex-clojure-comment-reader-macro (count)
    "Comment symex using clojure(script)'s comment reader macro"
    (interactive "p")
    (dotimes (_ count)
      (insert "#_")
...
arguments and only proceed with evaluation if it returns nil."
  ...
  (defun my-cider-reset ()
    ...
    (cider-interactive-eval-clj "(user/restart-system)"))

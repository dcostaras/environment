...
#+begin_src elisp :var name="" :var nl="" :var lang="" :noweb-ref no
...
`(symex--current-node-face :background ,(doom-darken (doom-color 'orange) 0.7))
#+end_src
**** save-file


#+begin_src lisp :noweb-ref evil-general
...
#+begin_src elisp :noweb-ref faces
`(evil-goggles-yank-face :background ,>evil-operator-state-colour)
`(evil-goggles-change-face :background ,>evil-insert-state-colour)
`(evil-goggles-paste-face :background ,>evil-normal-state-colour)
...
#+begin_src elisp :noweb-ref faces
`(region :background ,(doom-darken >evil-visual-state-colour 0.75))
#+end_src
...
(general-define-key
 ...
 "s-d" '+workspace/close-window-or-workspace
...
;; (advice-remove 'org-meta-return #'org-meta-return-around)
...
(defun >electric-forms-font-lock ()
  (font-lock-add-keywords
   nil
   `((,(rx "("
           (* (syntax whitespace))
...
(>define-clojure-mode-hooks #'>electric-forms-font-lock)
   #+end_src
...
(defun >cider-results-buffer-pprint (&rest r)
  (with-current-buffer cider-result-buffer
    (setq buffer-read-only nil)
    (>jet (point-min) (point-max))))

(advice-add 'cider-emit-into-popup-buffer :after #'>cider-results-buffer-pprint)
 #+end_src
...
#+begin_src clojure :noweb-ref no
...
(defun >cider-refresh-all ()
  (interactive)
  (projectile-save-project-buffers)
  (cider-interactive-eval-clj "(do (in-ns 'user) (refresh-all))"))
...
(defun >cider-load-debug-tools ()
  (interactive)
  (>cider-interactive-eval <<src-block-value(name="cider-load-debug-tools",lang="clojure")>>))
...
#+begin_src elisp
(defun >clojure-modes-completion-hook ()
  (prn completion-at-point-functions)
  (add-hook! 'completion-at-point-functions :local #'lsp-completion-at-point)
...
(add-hook! '(lsp-mode-hook lispy-mode-hook lispyville-mode-hook)
  (defun >lsp-mode-hook ()
    (add-hook! 'completion-at-point-functions :local #'lsp-completion-at-point)
    ;; (add-hook! 'completion-at-point-functions :local #'cape-dabbrev)
    (add-hook! 'completion-at-point-functions :local #'dabbrev-capf)
    (prn completion-at-point-functions)))
(>define-clojure-mode-hooks #'>clojure-modes-completion-hook)
#+end_src
...
**** One off functions
***** Humble
****** Card props -> dynamic card props
#+begin_src clojure :noweb-ref none :shebang "#!/usr/bin/env bb -i" :tangle ~/bin/humble-dynamic-card-props
(println
 (transduce
  (comp (map (fn [string]
               (-> string
                   (str/replace #":title" ":component.card/title")
                   (str/replace #":subtitle" ":component.card/subtitle")
                   (str/replace #":link-to" ":component.card/link")
                   (str/replace #":fa-icon" ":component.card/icon")
                   (str/replace #":powered-by" ":component.card/powered-by")
                   (str/replace #":type" ":component.card/type"))))
        (interpose "\n"))
  str
  ,*input*))
#+end_src
...
#+begin_src elisp
(defun >humble-namespace-card-props (beginning end)
  (interactive "r")
  (narrow-to-region beginning end)
  ...
  (shell-command-on-region
   (point-min)
   (point-max)
   "humble-dynamic-card-props"
   (current-buffer)
   t
   "*humble-dynamic-card-props error buffer*"
   t)
...
#+begin_src clojure :noweb-ref none :shebang "#!/usr/bin/env bb -i" :tangle ~/bin/css-to-hiccup
;; FIXME comments come through
;; FIXME newlines come through
(println
 (transduce
  (map (fn [string]
...
#+begin_src elisp
(defun >css-to-hiccup (beginning end)
  (interactive "r")
  (narrow-to-region beginning end)
...
* Rigpa
:PROPERTIES:
:header-args: :tangle ~/.config/rigpa-emacs/init.el
:END:
...
#+begin_src clojure :noweb-ref karabiner-rules
{:des "App launchers"
 :rules [[:!EQu "open /Applications/Emacs.app"]
         [:!EQi "open /Applications/Safari.app"]
         [:!EQo "open /Applications/iTerm.app"]
         [:!EQz "open /Applications/Slack.app"]
         [:!EQx "open /Applications/Linear.app"]]}
#+end_src
...
#+begin_src clojure :noweb-ref karabiner-rules
{:des "Safari"
 :rules [[:!Tj :down_arrow [:safari]]
         [:!Tk :up_arrow [:safari]]
         [[:left_option :l] :!CSclose_bracket [:safari]]
         [[:left_option :h] :!CSopen_bracket [:safari]]]}
#+end_src
...
{:des "caps_lock to esc when pressed alone, to ctrl as modifier"
         :rules [[:##caps_lock :left_control [:mb] {:alone :escape}]
                 [:##left_control :caps_lock [:mb]]]}
#+end_src
...
{:devices {:mb [{:vendor_id 1452 :product_id 834}]
           :ml [{:vendor_id 12951 :product_id 6505}]}

 :main [;; Newlines areneeded
        <<karabiner-rules>>
        ]
...
#+begin_src clojure :noweb-ref karabiner-rules
;; {:des "Move space left/right C-A-h/C-A-l "
;;  :rules [[:!OTh <<karabiner-shell(cmd="yabai -m space --move prev")>>]
;;          [:!OTl <<karabiner-shell(cmd="yabai -m space --move next")>>]]}
#+end_src
...
#+begin_src clojure :tangle ~/.config/deps.edn
;; {:override-deps {org.realifelearning/yggdrasil {:local/root "/Users/donavan/src/yggdrasil"}
;;                  org.realifelearning/frame {:local/root "/Users/donavan/src/rl-frame"}
;;                  org.realifelearning/belt {:local/root "/Users/donavan/src/rl-belt"}}}
#+end_src
...
takes 0.163 s
#+begin_src clojure :tangle ~/bin/b :shebang "#!/usr/bin/env bb"
(babashka.deps/add-deps
 '{:deps {org.realifelearning/bench {:local/root "/Users/donavan/src/bench"}}})
((requiring-resolve 'org.realifelearning.bench/start))
#+end_src

#+begin_src  clojure :tangle ~/.config/bench.edn
{:org.realifelearning.bench/tools
 {:active-tools #{:cider :reveal}}

 :org.realifelearning.bench.deps/default-aliases
 [:org.realifelearning.bench.tools/add-libs
  :org.realifelearning.bench.tools/misc-tools]

 :org.realifelearning.bench.system/targets
 [:org.realifelearning.bench.cli/component
  :org.realifelearning.bench.nrepl/component
  :org.realifelearning.bench.emacs/cider-connect
  ;; :org.realifelearning.bench.system/restart
  ]

 :org.realifelearning.bench.deps/local-deps
 {org.realifelearning/belt {:local/root "/Users/donavan/src/rl-belt"}
  org.realifelearning/frame {:local/root "/Users/donavan/src/rl-frame"}
  org.realifelearning/yggdrasil {:local/root "/Users/donavan/src/yggdrasil"}
  io.github.dcostaras/wax {:local/root "/Users/donavan/src/wax"}

  thheller/shadow-cljs {:local/root "/Users/donavan/src/shadow-cljs"}
  com.bhauman/rebel-readline {:local/root "/Users/donavan/src/rebel-readline/rebel-readline"}}}
  #+end_src
...
#+begin_src clojure :tangle ~/src/bench-test/_bench.edn
{:org.realifelearning.bench.tools/cider
 {:nrepl-port 7001
  ;; :cljs-nrepl? true
  }

 :org.realifelearning.bench.deps/active-aliases
 [:shadow-test :server-test
  ]

 ;; NOTE testing
 :org.realifelearning.bench.system/targets [:local.hai/system]
 :org.realifelearning.bench.deps/extra-deps {local/humble {:local/root "/Users/donavan/src/local-humble"}}
 ;; END NOTE

 :org.realifelearning.bench.monorepo/projects
 ;; TODO include should add path to config
 {:shadow-test #include "shadow-test/_bench.edn"
  :server-test #merge [#include "server-test/bench.edn"
                       #include "server-test/_bench.edn"]}
 }
#+end_src

**** shadow-test

#+begin_src clojure :tangle ~/src/bench-test/shadow-test/_bench.edn
{:org.realifelearning.bench.monorepo/project-path "shadow-test"
...
;;   }
 ...
 :org.realifelearning.bench.system/targets
 [:org.realifelearning.bench.shadow/watch]

 :org.realifelearning.bench.shadow/config
 {:org.realifelearning.bench.shadow.config/active-builds
  [{:build-id :frontend
    :build-type :release}
   :worker]

  :org.realifelearning.bench.shadow.config/build-type
  :compile}}
#+end_src

**** server-test

#+begin_src clojure :tangle ~/src/bench-test/server-test/bench.edn
{:some-key ["bench.edn"]}
#+end_src

#+begin_src clojure :tangle ~/src/bench-test/server-test/_bench.edn
{:some-other-key ["_bench.edn"]}
#+end_src

*** bench

#+begin_src clojure :tangle ~/src/bench/_bench.edn
{:org.realifelearning.bench.tools/cider
 {:nrepl-port 8001}}
#+end_src
*** earth

#+begin_src clojure :tangle ~/src/earth/_bench.edn
{:org.realifelearning.bench.tools/cider
 {:nrepl-port 6001}
...
#+begin_src clojure :tangle ~/src/powerblog/_bench.edn
{:org.realifelearning.bench.tools/cider
 {:nrepl-port 8001}}
#+end_src
...
**** Git hooks
[[id:9fa2bec3-8c56-4931-ad79-17c7979086f5][Git hook management]]

#+begin_src clojure :tangle ~/src/hai/.git/hooks/pre-commit :shebang #!/usr/bin/env bb
(ns git-hook)
...
**** Global bench config
#+begin_src clojure :tangle ~/src/hai/_bench.edn
{:org.realifelearning.bench.tools/cider
 {:nrepl-port 9000
  :cljs-nrepl? true}
 ...
 :org.realifelearning.bench.system/targets
 [;; :org.realifelearning.bench.shadow/watch
  ;; :org.realifelearning.bench.shadow.tap-support/runtime-taps
  :local.hai/system]
 ...
 [
  :web-extension
  :platform
  :transaction-classifier/main
  :transaction-classifier/local
  ;; :apps-script
 ...
 :org.realifelearning.bench.deps/local-deps
 {reagent/reagent {:git/url "https://github.com/GaiwanTeam/reagent"
                   :git/sha "b87e3afe5e71bc5697e866f56488167da0c47d19"}
  girouette/girouette {:git/url "https://github.com/GaiwanTeam/girouette"
                       :git/sha "267a4517f02f8b305fee35dfb163cba169d7b7f8"
                       :deps/root "lib/girouette"}
  com.owainlewis/salesforce {:local/root "/Users/donavan/src/salesforce"}
  hato/hato {:local/root "/Users/donavan/src/hato"}}

 :org.realifelearning.bench.deps/extra-deps
 {local/humble {:local/root "/Users/donavan/src/local-humble"}}

 :org.realifelearning.bench.monorepo/projects
 {
  :platform #include "projects/platform/_bench.edn"
  :web #include "projects/web-extension/_bench.edn"
  ;; :transaction-classifier #include "projects/transaction-classifier/_bench.edn"
  }}
#+end_src

**** platform

#+begin_src clojure :tangle ~/src/hai/projects/platform/_bench.edn
{:org.realifelearning.bench.deps/local-deps
 {com.owainlewis/salesforce {:local/root "/Users/donavan/src/salesforce"}}

 :org.realifelearning.bench.monorepo/active?
 true

 :org.realifelearning.bench.cli/commands
 {:start local.hai.platform/start
  :restart local.hai.platform/restart
  :stop local.hai.platform/stop}}

#+end_src

**** web-extension

#+begin_src clojure :tangle ~/src/hai/projects/web-extension/_bench.edn
{ ;; TODO automate from include
 :org.realifelearning.bench.monorepo/project-path
 "projects/web-extension"

 :org.realifelearning.bench.monorepo/active?
 true

 :org.realifelearning.bench.cli/commands
 {:start local.hai.web-extension/start
  :restart local.hai.web-extension/restart
  :stop local.hai.web-extension/stop
  :prerelease local.hai.web-extension/prerelease}

 :org.realifelearning.bench.shadow/config
 {:org.realifelearning.bench.shadow.config/active-builds
  [:popup :service-worker :content-script :options :sidebar :devcomponents]

  ;; TODO args ... possibly this or pathom params
  ;; [{:build-id :frontend
  ;;   :build-type :release}
  ;;  :worker]

  ;; TODO actually called 'mode' by shadow
  :org.realifelearning.bench.shadow.config/build-type
  :compile}}
#+end_src
...
#+begin_src clojure :tangle ~/src/hai/projects/transaction-classifier/_bench.edn
{ ;; TODO automate from include
 :org.realifelearning.bench.monorepo/project-path
 "projects/transaction-classifier"

 :org.realifelearning.bench.monorepo/active?
 true

 :org.realifelearning.bench.cli/commands
 {:start local.hai.transaction-classifier/start
  :restart local.hai.transaction-classifier/restart
  :stop local.hai.transaction-classifier/stop}
...
#+begin_src sh :tangle ~/.zshrc
source /Users/donavan/.config/op/plugins.sh
...
#+begin_src toml :tangle ~/.config/aerospace.toml
...
(->> apps
     (-map (fn (`(,app-name))
             (s-lex-format "yabai -m rule --add manage=off app='${app-name}'")))
     (s-join "\n"))
#+end_src
...
#+begin_src elisp :noweb-ref org-post-tangle-environment-hook
(>start-process "yabai --restart-service")
#+end_src
...
To re-enable nix things like karabiner run ~darwin-rebuild switch~
...
#+begin_src nix :tangle ~/.config/nix/darwin/configuration.nix
{ self }: { pkgs, ... }: {
...
# tmate GH runner debugging
# ssh zNTjmPPcUegsLZnM7DFZ2QuF5@sfo2.tmate.io
Host tmate tmate.io
User zNTjmPPcUegsLZnM7DFZ2QuF5
...
rm -rf ~/env && cd /tmp && curl --location --output env.zip https://github.com/dcostaras/environment/archive/refs/heads/master.zip && unzip env.zip && mv environment-master ~/env && cd ~/env && nix --extra-experimental-features "nix-command flakes" shell --impure --expr "{pkgs ? import <nixpkgs> {}}:pkgs.emacsWithPackages  (epkgs: (with epkgs.melpaStablePackages;    [dash s nix-mode ]))" --command emacs --batch --eval "(progn  (require 'org)  (require 's)  (require 'dash)  (require 'nix-mode)  (setq org-confirm-babel-evaluate nil)  (org-babel-tangle-file \"rc.org\" nil \"nix\"))"
#+end_src

*** Download and extract environment repo manually

#+name: env-extract
#+begin_src sh
cd /tmp &&
    curl --location --output env.zip https://github.com/dcostaras/environment/archive/refs/heads/master.zip &&
    unzip env.zip &&
    mv environment-master ~/env &&
    cd ~/env
#+end_src
...
(progn
  ...
  (org-babel-tangle-file "rc.org" nil "nix"))
#+end_src
...
rm -rf ~/env &&
    ...
    shell --impure --expr <<src-block-value(name="env-emacs",nl="nil")>> \
    --command emacs --batch --eval <<src-block-value(name="env-tangle",nl="nil")>>
#+end_src
...
cd ~/.config/nix/env &&
    XDG_CONFIG_HOME="$HOME/.config" \
    nix-build \
    -I "darwin=https://github.com/LnL7/nix-darwin/archive/master.tar.gz" \
    -I "darwin-config=$HOME/.config/nix/env/nix-darwin/configuration.nix" \
    https://github.com/LnL7/nix-darwin/archive/master.tar.gz -A installer
#+end_src
...
# NIX_PATH="$NIX_PATHdarwin=https://github.com/LnL7/nix-darwin/archive/master.tar.gz:darwin-config=$HOME/.config/nix/env/nix-darwin/configuration.nix" \

#+begin_src sh
cd ~/.config/nix/env &&
    XDG_CONFIG_HOME="$HOME/.config" \
    ./result/bin/darwin-installer -I "darwin=https://github.com/LnL7/nix-darwin/archive/master.tar.gz"
#+end_src

** Switch

#+begin_src sh
cd ~/.config/nix/env && sudo nix --extra-experimental-features "nix-command flakes" run nix-darwin -- switch --flake .
#+end_src

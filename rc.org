...
#+begin_src emacs-lisp :tangle ~/.doom.d/config.el :comments no
;;; -*- lexical-binding: t; -*-
#+end_src
...
(defun >tangle-environment ()
  ...
  (save-excursion
    (with-current-buffer (find-file my--environment-file-name)
      (save-buffer)
      (let* ((org-babel-exp-code-template "#+begin_src %lang :comments %comments :shebang %shebang :tangle %tangle%switches%flags\n%body\n#+end_src")
...
(defun >snippet-properties (directory)
  (s-join "\n" (list (s-lex-format ":tangle-dir: ~/.doom.d/snippets/${directory}/")
                     ":header-args+: :comments no")))
#+end_src

#+MACRO: snippet-dir (eval (>snippet-properties $1))
...
**** Snippets
:PROPERTIES:
{{{snippet-dir(emacs-lisp-mode)}}}
:END:

#+begin_src snippet :tangle (>tangle-dir "test")
# name: test
# key: test
# --
a test snippet

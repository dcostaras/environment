...
(defun >tangle-environment ()
  (interactive)
  (let (temp-org-file-name)
    (with-current-buffer (find-file-noselect my--environment-file-name)
      (save-buffer)
      (let* ((org-babel-exp-code-template "#+begin_src %lang :comments %comments :shebang %shebang :tangle %tangle%switches%flags\n%body\n#+end_src")
             (org-babel-default-header-args (->> org-babel-default-header-args
                                                 (cons '(:shebang . ""))
                                                 (cons '(:comments . "no")))))
        (setq temp-org-file-name (org-org-export-to-org))
        ;; (delete-file temp-org-file-name)
        ))
    (with-current-buffer (find-file-noselect temp-org-file-name)
      (let (org-babel-pre-tangle-hook
            (org-babel-tangle-finished-hook '(>post-tangle-environment-hook)))
...
#+begin_src elisp :noweb yes :tangle ~/.doom.d/config.el
(use-package! org
  :init
  <<org-init>>
  :config
...
#+begin_src elisp :noweb yes :noweb-prefix no :tangle ~/.doom.d/config.el
...
#+begin_src elisp :noweb yes :tangle ~/.doom.d/packages.el
;; -*- no-byte-compile: t; -*-
;;; $DOOMDIR/packages.el
...
(let* ((output-buffer (generate-new-buffer "*Async goku karabiner.edn compile*"))
       (proc (start-process "goku" output-buffer "goku"))

...
#+begin_src elisp :var name="" :noweb-ref no
(->> (save-excursion
...
(use-package! org-roam
  ...
  (defun >org-roam-capture-get-node-path ()
    (if-let ((olp (org-roam-node-olp org-roam-capture--node)))
        (append olp (list (org-roam-node-title org-roam-capture--node)))
      '()))
...
will be toggled."
    (let* ((remove-hook-fn (lambda (&rest args)
...
(defun >osa-safari-get-tab ()
  (->> (osa-eval <<src-block-value(name="safari-get-url")>> :lang "JavaScript")
       (>osa-parse-json)))
#+end_src
...
(use-package! yequake
  :config
  (nconc yequake-frames '(<<yequake-frames>>)))
#+end_src
...
**** Snippets
:PROPERTIES:
:tangle-dir: ~/.doom.d/snippets/emacs-lisp/
...
**** COMMENT my-clojure-def-and-name-regex
...
#+begin_src elisp :noweb-ref doom-config :noweb yes
(defun >org-capture-safari-tab ()
  (interactive)
  (let ((org-roam-capture-templates
         '(("d" "default" entry <<src-block-value(name="org-capture-safari-tab-template")>>
            ...
            :unnarrowed t))))
    (yequake-org-roam-capture nil nil :info (>osa-safari-get-tab))))
#+end_src

#+begin_src elisp :noweb-ref yequake-frames
("org-capture-safari-tab"
 (buffer-fns . (>org-capture-safari-tab))
 (width . 0.75)
 (height . 0.5)
 (alpha . 0.95)
...
{:des ""
 :rules [[:##right_shift :right_shift nil {:alone "e '(>org-capture-safari-tab-frame)'"}]]}
#+end_src
...
/opt/homebrew/bin/emacsclient -e -n "$@"
#+end_src

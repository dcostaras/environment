      ...
      (when ns
        (cider-interactive-eval
         ...
         (nrepl-make-response-handler
          code-buffer
          (lambda (_buffer test-var-name)
            (if test-var-name
                (progn
                  ;; Kaocha test
                  (kaocha-runner--run-tests
                   (kaocha-runner--testable-sym ns test-var-name (eq major-mode 'clojurescript-mode))
                   nil
                   t
                   code-buffer)
                  ;; Cider test
              ...
              (message "No test at point. Does your test var have ^:test meta?")))
          nil
          nil
          (lambda (_buffer)))))))
